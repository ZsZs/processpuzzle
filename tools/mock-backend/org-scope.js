...
// and `<orgKey>-state-machines`. `<orgKey>-widget-definitions` is the exception — a hand-written
// fixture in db.json, because base-widget-backend ships no YAML loader, so a seed file in its
// resources would be read by json-server alone and would drift from the Java backend unnoticed.
//
// base-workflow is not routed here at all: it is served by base-workflow-backend, and this mock is
// for third-party sources rather than for the platform's own features.
//
