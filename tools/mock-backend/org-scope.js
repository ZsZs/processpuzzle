...
// `<orgKey>-widget-definitions`, `<orgKey>-tools` and `<orgKey>-instances` are the exceptions — hand-written
// fixtures in db.json, because neither base-widget-backend nor base-workflow's tool and instance resources
// ship a YAML loader, so a seed file in their resources would be read by json-server alone and would drift
// from the Java backend unnoticed.
//
...
const ORG_SCOPED_PATH = /^\/organizations\/([a-z0-9][a-z0-9-]*)\/(rules|app-definitions|modules|widget-definitions|state-machines|processes|tools|instances)(?=[/?]|$)/;
