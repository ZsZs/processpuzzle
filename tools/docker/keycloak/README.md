...
| `processpuzzle-biz-realm.json` | `processpuzzle-biz` | `processpuzzle-biz` | The Biz applications on `http://localhost:9092`. The public SPA client uses authorization-code flow with PKCE. |
| `processpuzzle-admin-realm.json` | `processpuzzle-admin` | `processpuzzle-admin` | The staff administration app on `http://localhost:9091` / `http://localhost:4201`. Declares the `platform-admin` realm role and seeds a `platform-admin` / `platform-admin` user holding it. **The application itself is not built here** — it moved to the private `processpuzzle-biz` repository on 2026-09-04, while this realm stayed, because a realm is shared infrastructure rather than something a backend provisions for itself; see [Extracting platform-admin](../../../docs/platform-admin-extraction.md). Nothing in this repository's compose files logs into it. |
| `processpuzzle-custom-realm.json` | `processpuzzle-custom` | — | The shared Custom realm. Native Keycloak Organizations is enabled; the Admin application provisions organizations and members, while the Custom application deployment supplies its shell client configuration. |
| `processpuzzle-admin-realm.json` | `processpuzzle-admin` | `processpuzzle-admin`, `processpuzzle-biz` | The staff administration app on `http://localhost:9091` / `http://localhost:4201`, plus the Biz frontend's fallback client on `http://localhost:9092` / `http://localhost:4202`. Declares the `platform-admin` realm role and seeds a `platform-admin` / `platform-admin` user holding it. **The applications themselves are not built here** — they moved to the private `processpuzzle-biz` repository on 2026-09-04, while this realm stayed, because a realm is shared infrastructure rather than something a backend provisions for itself; see [Extracting platform-admin](../../../docs/platform-admin-extraction.md). Nothing in this repository's compose files logs into it. |
...
- **The admin realm's client id is not `keycloak.admin.tenant-client-id`.** The client here (`processpuzzle-admin`) is the public client the staff application authenticates with, named in that application's `src/run-time-conf/config.common.json` and `config.prod.json` — in the private repository since the extraction. `keycloak.admin.tenant-client-id` is a *different* thing: the client created inside each provisioned tenant realm, still `processpuzzle-biz`. It is a `KeycloakAdminProperties` default now rather than something this repository's `application.yaml` sets, because provisioning realms left with platform-admin.
...
The `master` realm's service account is *not* here: see `init/bootstrap-platform-admin-client.sh` and the comment at the top of it for why it cannot be. That script also grants the management roles for all four realms above, so their names have to match this table. It is named after the *client* it creates and stays regardless of the extraction: `org-admin` needs the same service account to manage users.
The `master` realm's service account is *not* here: see `init/bootstrap-platform-admin-client.sh` and the comment at the top of it for why it cannot be. That script also reconciles the Biz fallback client because realm imports do not merge into an existing realm, then grants the management roles for both realms above, so their names have to match this table. It is named after the *client* it creates and stays regardless of the extraction: `org-admin` needs the same service account to manage users.
