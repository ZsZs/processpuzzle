...
class EntityTranslationEndpointTest {

    private static final String ORG = "processpuzzle-testbed";

    private EntityTranslationRepository repository;
    private EntityTranslationEndpoint endpoint;

    @BeforeEach
    void setUp() {
        repository = mock(EntityTranslationRepository.class);
        endpoint = new EntityTranslationEndpoint(repository);
    ...
    private void seeded(String scope, String locale, Map<String, Object> messages) {
        when(repository.findById(new TranslationBundleKey(ORG, scope, locale)))
                .thenReturn(Optional.of(new EntityTranslationBundle(ORG, scope, locale, messages)));
    }
    ...
    void servesASeededScopedBundleAsItWasStored() {
        ...
        assertThat(endpoint.getEntityScopedTranslations(ORG, "a_scope", "en").getBody())
    ...
    void servesTheRootBundleFromTheRootScopeSentinel() {
        ...
        assertThat(endpoint.getEntityTranslations(ORG, "en").getBody()).isEqualTo(Map.of("home", "Home"));
    }
    ...
    void answersAnUnseededScopeWithAnEmptyBundleRatherThanNotFound() {
        var response = endpoint.getEntityScopedTranslations(ORG, "never_seeded", "en");
    ...
    void keepsTheLocalesOfOneScopeApart() {
        ...
        assertThat(endpoint.getEntityScopedTranslations(ORG, "a_scope", "en").getBody()).containsEntry("k", "English");
        assertThat(endpoint.getEntityScopedTranslations(ORG, "a_scope", "de").getBody()).containsEntry("k", "Deutsch");
    }
}
