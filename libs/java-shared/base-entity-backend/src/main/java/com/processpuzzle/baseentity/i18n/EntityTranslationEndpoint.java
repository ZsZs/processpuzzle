...
import com.processpuzzle.baseentity.api.BaseEntityTranslationsApi;
...
@RestController
@LogClass
public class EntityTranslationEndpoint implements BaseEntityTranslationsApi {

    private final TranslationBundleResponseProvider<EntityTranslationBundle> responseProvider;

    public EntityTranslationEndpoint(EntityTranslationRepository repository) {
        responseProvider = new TranslationBundleResponseProvider<>(repository::findById);
    }

    @Override
    @Transactional(readOnly = true)
    public ResponseEntity<Map<String, Object>> getEntityTranslations(String orgKey, String locale) {
        return bundle(orgKey, AbstractTranslationBundle.ROOT_SCOPE, locale);
    }

    @Override
    @Transactional(readOnly = true)
    public ResponseEntity<Map<String, Object>> getEntityScopedTranslations(String orgKey, String scope, String locale) {
