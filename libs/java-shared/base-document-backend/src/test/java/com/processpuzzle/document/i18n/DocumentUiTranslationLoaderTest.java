...
class DocumentUiTranslationLoaderTest {
    ...
    private DocumentUiTranslationLoader loader;

    @BeforeEach
    void setUp() {
        repository = mock(DocumentUiTranslationRepository.class);
        importer = mock(TranslationBundleImporter.class);
        loader = new DocumentUiTranslationLoader(repository, importer);
    ...
    void storesABundleThatIsNotThereYet() {
        ...
        ArgumentCaptor<DocumentUiTranslationBundle> saved = ArgumentCaptor.forClass(DocumentUiTranslationBundle.class);
    ...
    void mergesIntoABundleThatIsAlreadyThere() {
        DocumentUiTranslationBundle existing =
                new DocumentUiTranslationBundle(ORG, "a_scope", "en", Map.of("kept", "old", "shared", "old"));
    ...
    void namesItselfToTheImporterSoTheStartupLogCanTellTheSevenApart() {
        ...
        verify(importer).importAll(org.mockito.ArgumentMatchers.eq("base-document"), any());
    }
}
