...
class DocumentUiTranslationEndpointTest {

    private static final String ORG = "processpuzzle-testbed";

    private DocumentUiTranslationRepository repository;
    private DocumentUiTranslationEndpoint endpoint;

    @BeforeEach
    void setUp() {
        repository = mock(DocumentUiTranslationRepository.class);
        endpoint = new DocumentUiTranslationEndpoint(repository);
    ...
    private void seeded(String scope, String locale, Map<String, Object> messages) {
        when(repository.findById(new TranslationBundleKey(ORG, scope, locale)))
                .thenReturn(Optional.of(new DocumentUiTranslationBundle(ORG, scope, locale, messages)));
    }
    ...
    void servesASeededScopedBundleAsItWasStored() {
        ...
        assertThat(endpoint.getDocumentUiScopedTranslations(ORG, "a_scope", "en").getBody())
    ...
    void servesTheRootBundleFromTheRootScopeSentinel() {
        ...
        assertThat(endpoint.getDocumentUiTranslations(ORG, "en").getBody()).isEqualTo(Map.of("home", "Home"));
    }
    ...
    void answersAnUnseededScopeWithAnEmptyBundleRatherThanNotFound() {
        var response = endpoint.getDocumentUiScopedTranslations(ORG, "never_seeded", "en");
    ...
    void keepsTheLocalesOfOneScopeApart() {
        ...
        assertThat(endpoint.getDocumentUiScopedTranslations(ORG, "a_scope", "en").getBody()).containsEntry("k", "English");
        assertThat(endpoint.getDocumentUiScopedTranslations(ORG, "a_scope", "de").getBody()).containsEntry("k", "Deutsch");
    }
}
