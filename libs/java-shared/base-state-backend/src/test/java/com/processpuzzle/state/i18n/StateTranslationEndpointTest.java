package com.processpuzzle.state.i18n;
...
class StateTranslationEndpointTest {

    private static final String ORG = "processpuzzle-testbed";

    private StateTranslationRepository repository;
    private StateTranslationEndpoint endpoint;

    @BeforeEach
    void setUp() {
        repository = mock(StateTranslationRepository.class);
        endpoint = new StateTranslationEndpoint(repository);
    ...
    private void seeded(String scope, String locale, Map<String, Object> messages) {
        when(repository.findById(new TranslationBundleKey(ORG, scope, locale)))
                .thenReturn(Optional.of(new StateTranslationBundle(ORG, scope, locale, messages)));
    }
    ...
    void servesASeededScopedBundleAsItWasStored() {
        ...
        assertThat(endpoint.getStateScopedTranslations(ORG, "a_scope", "en").getBody())
    ...
    void servesTheRootBundleFromTheRootScopeSentinel() {
        ...
        assertThat(endpoint.getStateTranslations(ORG, "en").getBody()).isEqualTo(Map.of("home", "Home"));
    }
    ...
    void answersAnUnseededScopeWithAnEmptyBundleRatherThanNotFound() {
        var response = endpoint.getStateScopedTranslations(ORG, "never_seeded", "en");
    ...
    void keepsTheLocalesOfOneScopeApart() {
        ...
        assertThat(endpoint.getStateScopedTranslations(ORG, "a_scope", "en").getBody()).containsEntry("k", "English");
        assertThat(endpoint.getStateScopedTranslations(ORG, "a_scope", "de").getBody()).containsEntry("k", "Deutsch");
    }
}
