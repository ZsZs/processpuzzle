...
import com.processpuzzle.state.api.BaseStateTranslationsApi;
...
@RestController
@LogClass
public class StateTranslationEndpoint implements BaseStateTranslationsApi {

    private final TranslationBundleResponseProvider<StateTranslationBundle> responseProvider;

    public StateTranslationEndpoint(StateTranslationRepository repository) {
        responseProvider = new TranslationBundleResponseProvider<>(repository::findById);
    }

    @Override
    @Transactional(readOnly = true)
    public ResponseEntity<Map<String, Object>> getStateTranslations(String orgKey, String locale) {
        return bundle(orgKey, AbstractTranslationBundle.ROOT_SCOPE, locale);
    }

    @Override
    @Transactional(readOnly = true)
    public ResponseEntity<Map<String, Object>> getStateScopedTranslations(String orgKey, String scope, String locale) {
