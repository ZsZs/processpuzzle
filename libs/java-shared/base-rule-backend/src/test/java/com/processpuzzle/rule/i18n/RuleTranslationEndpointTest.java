...
class RuleTranslationEndpointTest {

    private static final String ORG = "processpuzzle-testbed";

    private RuleTranslationRepository repository;
    private RuleTranslationEndpoint endpoint;

    @BeforeEach
    void setUp() {
        repository = mock(RuleTranslationRepository.class);
        endpoint = new RuleTranslationEndpoint(repository);
    ...
    private void seeded(String scope, String locale, Map<String, Object> messages) {
        when(repository.findById(new TranslationBundleKey(ORG, scope, locale)))
                .thenReturn(Optional.of(new RuleTranslationBundle(ORG, scope, locale, messages)));
    }
    ...
    void servesASeededScopedBundleAsItWasStored() {
        ...
        assertThat(endpoint.getRuleScopedTranslations(ORG, "a_scope", "en").getBody())
    ...
    void servesTheRootBundleFromTheRootScopeSentinel() {
        ...
        assertThat(endpoint.getRuleTranslations(ORG, "en").getBody()).isEqualTo(Map.of("home", "Home"));
    }
    ...
    void answersAnUnseededScopeWithAnEmptyBundleRatherThanNotFound() {
        var response = endpoint.getRuleScopedTranslations(ORG, "never_seeded", "en");
    ...
    void keepsTheLocalesOfOneScopeApart() {
        ...
        assertThat(endpoint.getRuleScopedTranslations(ORG, "a_scope", "en").getBody()).containsEntry("k", "English");
        assertThat(endpoint.getRuleScopedTranslations(ORG, "a_scope", "de").getBody()).containsEntry("k", "Deutsch");
    }
}
