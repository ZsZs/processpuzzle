...
class RuleApiExceptionHandlerTest {
    ...
    void anUnknownRuleBecomes404() {
        ResponseEntity<Map<String, String>> response =
                handler.handleNotFound(new RuleNotFoundException("demo", "missing"));

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        assertThat(response.getBody()).containsEntry("error", "Rule not found: demo/missing");
    }

    @Test
    void aDuplicateRuleIdBecomes409() {
        ResponseEntity<Map<String, String>> response =
                handler.handleConflict(new RuleAlreadyExistsException("demo", "max-quantity"));

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CONFLICT);
        assertThat(response.getBody()).containsEntry("error", "Rule already exists: demo/max-quantity");
    }
}
