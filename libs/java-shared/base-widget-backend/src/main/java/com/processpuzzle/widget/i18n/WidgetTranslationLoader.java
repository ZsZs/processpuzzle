 ...
 * library owns instead of nothing. Gated behind {@code base-widget.loadDefaultTranslations=true}.
 *
 ...
 */
@Component
@ConditionalOnProperty(prefix = "base-widget", name = "loadDefaultTranslations", havingValue = "true")
public class WidgetTranslationLoader {

    private final TranslationBundleLoaderSupport<WidgetTranslationBundle> loaderSupport;

    public WidgetTranslationLoader(WidgetTranslationRepository repository, TranslationBundleImporter importer) {
        loaderSupport = new TranslationBundleLoaderSupport<>(
                importer, repository::findById, repository::save, WidgetTranslationBundle::new);
    }

    @EventListener(ApplicationReadyEvent.class)
    @Transactional
    public void loadDefaults() {
        loaderSupport.loadDefaults("base-widget");
    }
}
