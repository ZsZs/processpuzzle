...
```powershell
npm exec nx build processpuzzle-store
```

Or using Maven directly:

```powershell
mvn clean install -pl libs/java-shared/processpuzzle-store
```
...
```powershell
npm exec nx test processpuzzle-store
```
...
```powershell
npm exec nx lint processpuzzle-store
```
