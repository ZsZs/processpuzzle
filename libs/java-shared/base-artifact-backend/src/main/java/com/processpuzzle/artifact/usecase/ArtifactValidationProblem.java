...
/**
 * One thing wrong with a candidate artifact. {@link Severity} is base-rule's own enum, reused
 ...
 */
public record ArtifactValidationProblem(String path, String errorId, String errorText, Severity severity)
        implements Serializable {

    private static final long serialVersionUID = 1L;

    public ArtifactValidationProblem(String path, String errorId, String errorText) {
    ...
    public static List<ArtifactValidationProblem> blocking(List<ArtifactValidationProblem> problems) {
        return problems == null
                ? List.of()
                : problems.stream().filter(ArtifactValidationProblem::blocksPersisting).toList();
    }
}
