...
class WorkflowTranslationLoaderTest {
    ...
    private WorkflowTranslationLoader loader;

    @BeforeEach
    void setUp() {
        repository = mock(WorkflowTranslationRepository.class);
        importer = mock(TranslationBundleImporter.class);
        loader = new WorkflowTranslationLoader(repository, importer);
    ...
    void storesABundleThatIsNotThereYet() {
        ...
        ArgumentCaptor<WorkflowTranslationBundle> saved = ArgumentCaptor.forClass(WorkflowTranslationBundle.class);
    ...
    void mergesIntoABundleThatIsAlreadyThere() {
        WorkflowTranslationBundle existing =
                new WorkflowTranslationBundle(ORG, "a_scope", "en", Map.of("kept", "old", "shared", "old"));
    ...
    void namesItselfToTheImporterSoTheStartupLogCanTellTheSevenApart() {
        ...
        verify(importer).importAll(org.mockito.ArgumentMatchers.eq("base-workflow"), any());
    }
}
