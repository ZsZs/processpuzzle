 ...
 * {@link WorkflowInstance} — put every completion through that entity's optimistic lock.
 */
class WorkflowContextTest {

    @Test
    void assembleStartsFromTheInstancesInitialContext() {
        WorkflowInstance instance = instanceWith(Map.of("orderId", "o-1"));

        assertThat(WorkflowContext.assemble(instance, List.of())).containsExactlyEntriesOf(Map.of("orderId", "o-1"));
    }
    ...
    void assembleFoldsContributionsOverTheInitialContext() {
        ...
        assertThat(WorkflowContext.assemble(instance, tasks))
    ...
    void alaterContributionOverwritesAnEarlierOneAndTheInitialValue() {
        WorkflowInstance instance = instanceWith(Map.of("state", "DRAFT"));
        List<TaskInstance> tasks = List.of(
        ...
        assertThat(WorkflowContext.assemble(instance, tasks)).containsEntry("state", "APPROVED");
    }
    ...
    void contributionsWithTheSameTimestampFoldInTaskIdOrder() {
        ...
        assertThat(WorkflowContext.assemble(instance, ascending)).containsEntry("winner", "zzz");
        assertThat(WorkflowContext.assemble(instance, descending)).containsEntry("winner", "zzz");
    }
    ...
    void assembleIgnoresTasksThatHaveNotCompleted() {
        ...
        assertThat(WorkflowContext.assemble(instance, List.of(active, skipped)))
    ...
    void assembleToleratesNullsEverywhere() {
        ...
        WorkflowInstance nullInitial = WorkflowInstance.builder().build();
        nullInitial.setInitialContext(null);

        assertThat(WorkflowContext.assemble(null, null)).isEmpty();
        assertThat(WorkflowContext.assemble(nullInitial, null)).isEmpty();
        assertThat(WorkflowContext.assemble(nullInitial, List.of(nullContribution))).isEmpty();
    }

    /** The assembled map is a fresh copy; mutating it must not reach back into the entities. */
    @Test
    void assembleReturnsACopy() {
        WorkflowInstance instance = instanceWith(new HashMap<>(Map.of("orderId", "o-1")));

        WorkflowContext.assemble(instance, List.of()).put("injected", true);
    ...
    void contributionIsTheEntriesAddedOrChanged() {
        ...
        assertThat(WorkflowContext.contributionOf(before, after))
    ...
    void contributionIsEmptyWhenNothingChanged() {
        ...
        assertThat(WorkflowContext.contributionOf(unchanged, unchanged)).isEmpty();
        assertThat(WorkflowContext.contributionOf(null, null)).isEmpty();
        assertThat(WorkflowContext.contributionOf(Map.of("a", 1), null)).isEmpty();
    }
    ...
    void settingAKeyToNullCountsAsAContribution() {
        ...
        assertThat(WorkflowContext.contributionOf(before, after)).containsExactlyEntriesOf(after.entrySet().stream()
    ...
    void aMissingKeyIsNotTreatedAsARemoval() {
        assertThat(WorkflowContext.contributionOf(Map.of("orderId", "o-1", "state", "DRAFT"), Map.of("orderId", "o-1")))
                .isEmpty();
    }
    ...
    void contributionFoldsBackToTheWorkingContext() {
        WorkflowInstance instance = instanceWith(Map.of("orderId", "o-1"));
        Map<String, Object> inherited = WorkflowContext.assemble(instance, List.of());

        Map<String, Object> working = new HashMap<>(inherited);
        working.put("reviewedBy", "clerk");
        working.put("orderId", "o-1-corrected");

        TaskInstance completed = completed("review", 100, WorkflowContext.contributionOf(inherited, working));
