...
class WorkflowTranslationEndpointTest {

    private static final String ORG = "processpuzzle-testbed";

    private WorkflowTranslationRepository repository;
    private WorkflowTranslationEndpoint endpoint;

    @BeforeEach
    void setUp() {
        repository = mock(WorkflowTranslationRepository.class);
        endpoint = new WorkflowTranslationEndpoint(repository);
    ...
    private void seeded(String scope, String locale, Map<String, Object> messages) {
        when(repository.findById(new TranslationBundleKey(ORG, scope, locale)))
                .thenReturn(Optional.of(new WorkflowTranslationBundle(ORG, scope, locale, messages)));
    }
    ...
    void servesASeededScopedBundleAsItWasStored() {
        ...
        assertThat(endpoint.getWorkflowScopedTranslations(ORG, "a_scope", "en").getBody())
    ...
    void servesTheRootBundleFromTheRootScopeSentinel() {
        ...
        assertThat(endpoint.getWorkflowTranslations(ORG, "en").getBody()).isEqualTo(Map.of("home", "Home"));
    }
    ...
    void answersAnUnseededScopeWithAnEmptyBundleRatherThanNotFound() {
        var response = endpoint.getWorkflowScopedTranslations(ORG, "never_seeded", "en");
    ...
    void keepsTheLocalesOfOneScopeApart() {
        ...
        assertThat(endpoint.getWorkflowScopedTranslations(ORG, "a_scope", "en").getBody()).containsEntry("k", "English");
        assertThat(endpoint.getWorkflowScopedTranslations(ORG, "a_scope", "de").getBody()).containsEntry("k", "Deutsch");
    }
}
