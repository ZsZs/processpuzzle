 ...
 * library owns instead of nothing. Gated behind {@code base-workflow.loadDefaultTranslations=true}.
 *
 ...
 */
@Component
@ConditionalOnProperty(prefix = "base-workflow", name = "loadDefaultTranslations", havingValue = "true")
public class WorkflowTranslationLoader {

    private final TranslationBundleLoaderSupport<WorkflowTranslationBundle> loaderSupport;

    public WorkflowTranslationLoader(WorkflowTranslationRepository repository, TranslationBundleImporter importer) {
        loaderSupport = new TranslationBundleLoaderSupport<>(
                importer, repository::findById, repository::save, WorkflowTranslationBundle::new);
    }

    @EventListener(ApplicationReadyEvent.class)
    @Transactional
    public void loadDefaults() {
        loaderSupport.loadDefaults("base-workflow");
    }
}
