 ...
 * ArtifactInstance} as cascaded child collections: tasks within a workflow instance are
 ...
 * {@code workflowInstanceId} foreign keys, not JPA {@code @OneToMany} ownership.
 *
 ...
 * own contribution lives on its {@link TaskInstance} — see {@link WorkflowContext}. This row is
 * therefore written only when the <em>instance</em> changes state (started, completed, cancelled),
 * which no two tasks do at once.
 *
 * <p>{@code workflowId}/{@code workflowName} are denormalized copies taken at
 * start time — a workflow instance keeps running under the definition version it started with even
 * if the definition is edited afterward (definitions may not even be deletable while instances are
 * active, per {@code DeleteWorkflowUseCase}, but they remain editable).
 */
@Getter
@Setter
@Builder
...
public class WorkflowInstance {
     ...
     * that produced it and folded back in by {@link WorkflowContext#assemble}. Written once, which is
