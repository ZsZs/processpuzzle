 ...
 * names, fetched from the catalog and paired back up with the process's own assignments.
 *
 ...
 * advancing an instance needs "the third task of this process and who performs it" as one thing,
 * and would otherwise repeat the same join at four call sites.
 *
 * <p>{@link #tasks()} is in assignment order, which is the order the process author put them in and
 * the order {@code TaskActivationService} treats as sequential when a task is not
 * {@link TaskUse#isParallel()}.
 */
public record ResolvedProcess(
        Workflow definition,
        List<RoleDefinition> roles,
        List<ArtifactDefinition> artifacts,
        List<ResolvedTask> tasks) {

    /** One task of the process: the shared definition, plus its place in this process. */
