...
describe('createStateDescriptor', () => {
  ...
  it('roots the labels under the library scope', () => {
    ...
    expect(byName('terminal')?.i18nKey()).toBe('base_state.state_machine_state.terminal');
  });

  it('describes the state, its two flags and its opaque UI hints', () => {
    expect(attrs.map((attr) => attr.attrName)).toEqual(['key', 'name', 'terminal', 'locked', 'description', 'metadata']);
  });
  ...
  it('offers both flags as checkboxes', () => {
    expect(byName('terminal')?.formControlType).toBe(FormControlType.CHECKBOX);
    expect(byName('locked')?.formControlType).toBe(FormControlType.CHECKBOX);
  });
  ...
  // Which states are terminal and which are locked is the shape of the machine's boundary; reading it
  ...
  it('shows the identity and both flags in the list', () => {
    ...
    expect(tableColumns).toEqual(['key', 'name', 'terminal', 'locked']);
  });
});
