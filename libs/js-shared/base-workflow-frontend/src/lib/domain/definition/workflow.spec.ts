...
describe('Workflow', () => {
  ...
  // The catalog entities are referenced, not carried: what the workflow holds is ids, so nothing here
  // can drift from the record under `/roles` or `/artifacts`.
  it('holds its catalog references as plain ids', () => {
    const workflow = new Workflow({ roles: ['clerk'], artifacts: ['order-entity'], tools: ['automated-check-tool'] });

    expect(workflow.roles).toEqual(['clerk']);
    expect(workflow.artifacts).toEqual(['order-entity']);
