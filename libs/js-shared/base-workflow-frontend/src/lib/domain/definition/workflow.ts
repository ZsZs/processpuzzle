 ...
 *
 * A workflow **composes catalog entities it does not own**. Its roles, artifacts and tools are id
 * lists pointing at `/roles`, `/artifacts` and `/tools`, each authored once per tenant and shared
 * across workflows — so the same `Order Clerk` or `Fulfillment Invoice` is one record, not one per
 * workflow that mentions it. Only {@link WorkflowTaskAssignment} is nested, because an assignment has
 * no meaning outside the workflow making it.
 *
 * `PUT /workflows/{workflowId}` is a full replacement, so every list travels on every save — an absent
 * one is an emptied workflow, not an untouched one. `WorkflowMapper` is what keeps the id
 ...
 * `taskDefinitionId` is what identifies it within the workflow. Same arrangement as
...
export class Workflow implements BaseEntity {
  ...
  name: string;
  description?: string;
  /** Id of a parent workflow to inherit roles, artifacts and task assignments from. */
  extends?: string;
  /** Ids of the `Workflow Role Definition`s taking part in this workflow. */
  roles: string[];
  /** Ids of the `Artifact Definition`s this workflow produces or consumes. */
  artifacts: string[];
  /** Ids of the `Tool Definition`s this workflow's steps may invoke. */
  tools: string[];
