...
function createWorkflowAttrDescriptors(): AbstractAttrDescriptor[] {
  ...
  // A workflow this one inherits roles, artifacts and task assignments from — another row of *this*
  // list, so a real reference the framework can resolve and navigate to. Same shape as base-rule's
  // `extendsRuleId`, and the backend's `WorkflowExtendsValidator` refuses a cycle.
  ...
  // each shared across workflows, so what the workflow holds is an *id list*: adding a row picks an
  ...
  //
  // `referenceIdField` is left at its default: all three are keyed by `id`. `WorkflowMapper`
  ...
  // meaning outside this workflow. The rows travel inside this entity's payload and are saved with it,
