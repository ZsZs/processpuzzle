...
import { createProcessDefinitionDescriptor } from './process-definition.descriptors';
import {
...
describe('createProcessDefinitionDescriptor', () => {
  const descriptor = createProcessDefinitionDescriptor();
  ...
  it('names the entity so that the route segment follows from it', () => {
    expect(descriptor.entityName).toBe(PROCESS_DEFINITION_ENTITY_NAME);
  });
  ...
  it('roots the labels under the library scope', () => {
    expect(descriptor.scopeRoot()).toBe('base_workflow.process_definition');
    expect(descriptor.i18nKey()).toBe('base_workflow.process_definition._self');
    expect(byName('tasks')?.i18nKey()).toBe('base_workflow.process_definition.tasks');
  });
  ...
  // `entityName` — so nothing has to be mirrored and the list's own column opens the process.
  it('identifies a process by its author-chosen id', () => {
  ...
  it('offers `extends` as a reference to another process, so a cycle is at least visible', () => {
    expect(byName('extends')?.formControlType).toBe(FormControlType.FOREIGN_KEY);
    expect(byName('extends')?.linkedEntityType).toBe(PROCESS_DEFINITION_ENTITY_NAME);
  });
  ...
  // The one thing a process still owns: an assignment pairs a shared task with the role performing it
  // *here*, so it has no meaning outside this process and travels inside its payload.
  it('carries only the task assignments as embedded components of itself', () => {
    ...
    expect(descriptor.embeddedAttrFor(PROCESS_TASK_ASSIGNMENT_ENTITY_NAME)?.attrName).toBe('tasks');
  });

  // An assignment has no `id` of its own; the task it assigns is what identifies it within the process.
