...
import { ProcessInstanceStore } from './process-instance.store';
import { OTHER_PROCESS_INSTANCE_DTO, pageOfProcessInstances, PROCESS_INSTANCE_DTO } from './test-process-instance';

describe('ProcessInstanceStore', () => {
  const serviceRoot = 'http://localhost:3000/organizations/processpuzzle-testbed';
  let store: InstanceType<typeof ProcessInstanceStore>;
  ...
  beforeEach(() => {
    ...
    store = TestBed.inject(ProcessInstanceStore);
    controller = TestBed.inject(HttpTestingController);
    controller.expectOne(`${serviceRoot}/instances`).flush(pageOfProcessInstances(PROCESS_INSTANCE_DTO, OTHER_PROCESS_INSTANCE_DTO));
  });
  ...
  it('exposes the whole run as current entity', () => {
    ...
    expect(store.currentEntity()?.processDefinitionName).toBe('Order Fulfillment Workflow');
