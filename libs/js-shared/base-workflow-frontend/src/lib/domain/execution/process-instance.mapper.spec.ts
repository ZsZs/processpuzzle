...
import { ProcessInstance, ProcessInstanceStatus, TaskInstance, TaskInstanceStatus, ArtifactInstance } from './process-instance';
import { ProcessInstanceMapper } from './process-instance.mapper';
import { OTHER_PROCESS_INSTANCE_DTO, PROCESS_INSTANCE_DTO } from './test-process-instance';
import { ArtifactType } from '../definition/artifact-definition';

describe('ProcessInstanceMapper', () => {
  const mapper = new ProcessInstanceMapper();

  describe('fromDto', () => {
    const instance = mapper.fromDto(PROCESS_INSTANCE_DTO);

    it('reads the header of a running instance', () => {
      expect(instance.id).toBe('8f14e45f-ceea-467a-9c9b-9b0c1f0f5a01');
      expect(instance.processDefinitionId).toBe('order-fulfillment-workflow');
      expect(instance.processDefinitionName).toBe('Order Fulfillment Workflow');
      expect(instance.status).toBe(ProcessInstanceStatus.ACTIVE);
    ...
    it('reads the failure path — a blocked task and a step that errored', () => {
      const [blocked] = mapper.fromDto(OTHER_PROCESS_INSTANCE_DTO).tasks;
  ...
  describe('toDto', () => {
    it('round-trips the instance without losing a nested row', () => {
      const dto = mapper.toDto(mapper.fromDto(PROCESS_INSTANCE_DTO));
    ...
    it('emits both nested lists unconditionally', () => {
      const dto = mapper.toDto(new ProcessInstance({ id: 'i1' }));
    ...
    it('emits exactly the contract’s fields and nothing else', () => {
      ...
      expect(Object.keys(dto).sort()).toEqual(['completedAt', 'context', 'entityId', 'id', 'processDefinitionId', 'processDefinitionName', 'startedAt', 'status', 'tasks', 'artifacts']);
    });
  });
});
