...
import { PROCESS_INSTANCE_I18N_SCOPE } from '../../base-workflow.i18n';
import { ARTIFACT_INSTANCE_ENTITY_NAME, PROCESS_INSTANCE_ENTITY_NAME, TASK_INSTANCE_ENTITY_NAME } from '../workflow-entity-names';
import { ProcessInstanceStatus } from './process-instance';
...
export { PROCESS_INSTANCE_ENTITY_NAME };

const processInstanceStatusSelectables = toSelectables(Object.keys(ProcessInstanceStatus));

function createProcessInstanceAttrDescriptors(): AbstractAttrDescriptor[] {
  ...
  const processDefinitionNameAttr = readOnlyAttr('processDefinitionName', FormControlType.TEXT_BOX, 'Process Name');
  processDefinitionNameAttr.isHeading = true;

  const statusAttr = readOnlyAttr('status', FormControlType.DROPDOWN, 'Status', processInstanceStatusSelectables);

  const processDefinitionIdAttr = readOnlyAttr('processDefinitionId', FormControlType.TEXT_BOX, 'Process');
  processDefinitionIdAttr.hideInTable = true;
  ...
  const identityRow = new FlexboxDescriptor([processDefinitionNameAttr, statusAttr, entityIdAttr], FlexDirection.ROW);
  identityRow.style = { 'column-gap': '10px' };
  const referenceRow = new FlexboxDescriptor([idAttr, processDefinitionIdAttr], FlexDirection.ROW);
...
export function createProcessInstanceDescriptor(): BaseEntityDescriptor {
  return new BaseEntityDescriptor({
    entityName: PROCESS_INSTANCE_ENTITY_NAME,
    attrDescriptors: createProcessInstanceAttrDescriptors(),
    i18nScope: PROCESS_INSTANCE_I18N_SCOPE,
    // Names the run in the status bar. Without it the bar would take the `isLinkToDetails` attribute,
    // which here is the UUID.
    titleKey: 'processDefinitionName',
