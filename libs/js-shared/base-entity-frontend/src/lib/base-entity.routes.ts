 ...
 */
export function baseEntityRoutes(embeddedChildren: EmbeddedChildRoute[] = [], isEmbedded = false): Routes {
  return [
    { path: '', redirectTo: BaseUrlSegments.ListForm, pathMatch: 'full' },
    {
      ...
      ...(isEmbedded ? { canActivate: [embeddedAggregateGuard] } : {}),
...
function embeddedChildRoute(child: EmbeddedChildRoute): Route {
  return {
    ...
    loadChildren: () => baseEntityRoutes(child.children?.() ?? [], true),
  };
}
// endregion
