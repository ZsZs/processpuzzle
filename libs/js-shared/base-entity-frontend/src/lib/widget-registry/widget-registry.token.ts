 ...
 * base-app-frontend and base-artifact-frontend both need it and neither depends on the other,
 ...
 * second, different meaning of "widget" in the same workspace, on top of the existing
 * ArtifactAttr/FormControlType.ARTIFACT collision with base-artifact's own "Artifact". Worth a
 * naming pass across all three.
 */

/**
 * Maps a widget registry key (WidgetRef.type / ArtifactBlock.type) to the Angular component
 * that renders it. Populated by each feature lib's own provider function — this file never
 * imports a concrete widget component itself, so app-shell, base-artifact, and any future
 ...
 * app-shell or base-artifact directly, which would put them in the business of knowing about
