...
export class BaseArtifactService extends BaseEntityRestService<Artifact> {
  constructor(protected override entityMapper: BaseArtifactMapper) {
    super(entityMapper, 'ARTIFACT_SERVICE_ROOT', 'artifacts');
  }

  /**
   * Every save from the generic form arrives here, and is redirected to
   * {@link updateProperties} instead of the inherited `PUT /artifacts/{id}`. The base
   * implementation serializes the whole entity through `toDto()`, and `updateArtifact` replaces
   * the stored block list wholesale — so a Properties save would overwrite `blocks` with whatever
   * the form happened to load, discarding every block-level edit `ArtifactContentService` made in
   * the meantime. Routing through the properties endpoint makes that structurally impossible
   * rather than merely unlikely: `ArtifactPropertiesInput` has no blocks field.
   *
   ...
   */
  override update(entity: PersistedEntity<Artifact>): Observable<PersistedEntity<Artifact>> {
    return from(this.updateProperties(entity.id, entity.title, entity.description, entity.inputPorts, entity.outputPorts));
  }

  /** Hits `PUT .../artifacts/{id}/properties`. Also callable directly, outside the form flow. */
  async updateProperties(id: string, title: string, description: string | undefined,
                          inputPorts: ArtifactInputPort[], outputPorts: ArtifactOutputPort[]): Promise<PersistedEntity<Artifact>> {
    ...
    return this.entityMapper.fromDto(dto) as PersistedEntity<Artifact>;
  }
}
