...
export interface WidgetEmbedNodeDeps {
  ...
  blocksById: Signal<ReadonlyMap<string, ArtifactBlock>>;
  resolveBinding: (portName: string) => unknown;
}
...
export function createWidgetEmbedExtension(deps: WidgetEmbedNodeDeps) {
  return Node.create({
    ...
    renderHTML({ HTMLAttributes }) {
      ...
      // (ArtifactBlock.editable === false) or server-side export — the live NodeView below is
...
class WidgetEmbedNodeView implements NodeView {
  ...
  private render(blocksById: ReadonlyMap<string, ArtifactBlock>) {
    ...
    if (!block || block.placement !== WidgetPlacement.REFERENCED) {
      ...
      // broken reference, since validateArtifact/replaceArtifactBlock will catch it on save —
  ...
  private applyProps(block: ArtifactBlock) {
    ...
    // is the artifact author saying "this one comes from the host," not a fallback.
