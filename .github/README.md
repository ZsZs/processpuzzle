...
For `build-processpuzzle-testbed-frontend.yml`, after the build the Spring Boot backend jar is built (`npx nx run processpuzzle-testbed-backend:build --no-cloud`) and `hoverkraft-tech/compose-action` brings up the two base compose files (`docker-compose-infrastructure.yaml`, `docker-compose-apps.yaml` — both pull-only, since they are what Coolify reads) plus the two overlays that restore what CI needs from them (`docker-compose-build.yaml` for the `build:` sections, `docker-compose-apps-local.yaml` for a non-external network and the cross-layer `depends_on`), with `--env-file tools/docker/env/infrastructure/.env.ci --env-file tools/docker/env/testbed/.env.ci` (one file per Coolify resource; repeated `--env-file` is additive), so the Playwright suite can run against a real stack. **The overlays must be listed after the base files** — the last file to name a key wins. Publishing the application images is [`build-testbed-apps.yml`](workflows/build-testbed-apps.yml)’s job, and it publishes to GHCR.
...
### Integration Test
Runs in `deploy-testbed-apps.yml` against the deployed environment, using the same `e2e-test` action with `target_environment` set to `stage` or `prod`. Skipped unless the environment declares `TESTBED_FRONTEND_PUBLIC_URL` — which must name the same origin as `apps/processpuzzle-testbed-e2e/env/.env.<environment>`, since that file is where the suite reads its base URL from. On `STAGE` that is `https://testbed.stage.processpuzzle.de`.
...
`build-testbed-apps.yml` builds the Angular bundle and the Spring Boot jar, packages each into an image and pushes `ghcr.io/zszs/processpuzzle-testbed-{frontend,backend}` tagged `sha-<commit>` (plus `latest` on `develop`), then calls `deploy-testbed-apps.yml`, which promotes those digests to `:stage` with `docker buildx imagetools create` and fires `COOLIFY_WEBHOOK_TESTBED` through [`coolify-deploy`](actions/coolify-deploy/action.yml). Promoting a verified build to production is the same workflow dispatched with `environment: PROD` and that `sha-<commit>`.

**One webhook, not two.** The testbed stack is a single Coolify **Docker Compose** resource holding both halves — a resource reads exactly one compose file, `tools/docker/docker-compose-apps.yaml`, and its webhook redeploys the whole thing. So ordering the backend ahead of the frontend is compose's `depends_on`, not a workflow step, and a missing secret **fails** the job rather than warning: [run 34049062656](https://github.com/ZsZs/processpuzzle/actions/runs/34049062656) was green while deploying nothing, which is the failure mode worth removing. See [`docs/build-deploy-strategy.md`](../docs/build-deploy-strategy.md) §4.
...
**Authentication is [npm Trusted Publishing](https://docs.npmjs.com/trusted-publishers) (OIDC) for every JS package.** Under OIDC the job's `id-token: write` permission lets the runner mint a token that npm exchanges for a short-lived publish credential, and provenance attestations are generated automatically (no `--provenance` flag, no `NPM_CONFIG_PROVENANCE`). `secrets.NPM_TOKEN` is no longer read by any release workflow — it exists only to bootstrap a new package name.

That bootstrap is the one case OIDC cannot cover. A trusted publisher is registered on a package's own settings page on npmjs.com, and that page only exists once the package does — so **the first publish of a new name cannot use OIDC** ([npm/cli#8544](https://github.com/npm/cli/issues/8544)). It fails with `This command requires you to be logged in`, the same message a missing publisher registration produces; the two are indistinguishable from the log. So when adding a library, publish `0.0.1` once with the token form — `registry-url` on `setup-node` plus `NODE_AUTH_TOKEN` / `NPM_CONFIG_PROVENANCE` on both that step and the publish step — then register the trusted publisher per the table at the end of this section and switch the workflow back to the OIDC form.

**Checking which auth a package last used, without logging in:** the registry's search API reports how the latest version was authenticated, which is the only unauthenticated view of this:
```bash
curl -s "https://registry.npmjs.org/-/v1/search?text=%40processpuzzle&size=50" \
  | jq -r '.objects[].package | select(.name|startswith("@processpuzzle/"))
           | "\(.name)@\(.version)\t\(if .publisher.trustedPublisher then "OIDC" else "token" end)"'
```
Note what this does and does not prove: `OIDC` means the registration was live and worked at that publish; `token` only means the *last* publish predated the switch, not that no publisher is registered. Note also that the registry's packument endpoint (`/@scope/name`) serves a cached 404 for some minutes after a name's first publish, while `/@scope/name/<version>` is already correct — don't read the former as a failed publish.
...
In the OIDC workflows the `Install Node` step deliberately omits `registry-url`: `actions/setup-node` would otherwise write an `.npmrc` containing a `_authToken` placeholder, and a configured (dead or empty) token takes precedence over OIDC. The corollary for the five token workflows is that they must *keep* `registry-url` — without it nothing writes the `.npmrc` and `NPM_TOKEN` is never read, however recently it was renewed.
