...
For `build-processpuzzle-testbed-frontend.yml`, after the build the Spring Boot backend jar is built (`npx nx run processpuzzle-testbed-backend:build --no-cloud`) and `hoverkraft-tech/compose-action` brings up the two base compose files (`docker-compose-infrastructure.yaml`, `docker-compose-apps.yaml` — both pull-only, since they are what Coolify reads) plus the two overlays that restore what CI needs from them (`docker-compose-build.yaml` for the `build:` sections, `docker-compose-apps-local.yaml` for a non-external network and the cross-layer `depends_on`), with `--env-file tools/docker/env/.env.ci`, so the Playwright suite can run against a real stack. **The overlays must be listed after the base files** — the last file to name a key wins. Publishing the application images is [`build-testbed-apps.yml`](workflows/build-testbed-apps.yml)’s job, and it publishes to GHCR.
