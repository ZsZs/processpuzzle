...
describe('HeaderComponent', () => {
  const testConfig: TranslocoTestConfig = { translations: { en: { navigation: { home: 'Home' } } } };
  ...
  beforeEach(async () => {
    ...
    const result = await setUpTranslocoTestBed(HeaderComponent, testConfig, {
      providers: [LayoutService, provideRouter([]), { provide: AUTHENTICATION_SERVICE, useValue: { isAuthenticated: signal(false) } }],
    });
  ...
  it('offers only titled routes for navigation', () => {
    expect(component.routes.length).toBeGreaterThan(0);
    expect(component.routes.every((route) => route.title)).toBeTruthy();
  });
