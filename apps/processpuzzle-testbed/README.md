...
**1. Build-time environment** — `src/environments/environment.<stage>.ts`

Auto-generated by `src/assets/setEnv.ts` from `.env` and CLI args. Each file holds only two values used as a fallback when the runtime layer is unavailable:
...
The Dockerfile at `tools/docker/testbed/Dockerfile` is **stage-agnostic** — it just bundles the built Angular app under nginx. At container start, `docker-entrypoint.sh` runs:
...
```
build-time:  setEnv.ts ──> environment.<stage>.ts   (FIREBASE_API_KEY, PIPELINE_STAGE)
                                  │ (fallback only)
container start:                  ▼
docker-compose env vars ──envsubst──> assets/runtime-env.json
                                  │
                                  ▼
